...
public class IncidentTest {
  ...
  public void shouldCreateAndResolveInputMappingIncident() {
    ...
    // Incident resolving which key/id we exactly need to update
    // the payload and resolve the incident?
    final AtomicLong activityId = new AtomicLong(0);
    assertElementInState(
        WorkflowInstanceIntent.ELEMENT_READY,
        "failingTask",
        (task) -> activityId.set(task.getScopeInstanceKey()));

    // when
    workflowClient.newUpdatePayloadCommand(activityId.get()).payload(PAYLOAD).send().join();

    // then
    assertJobCreated("failingTask");
  ...
  private static final class ControllableJobHandler implements JobHandler {
    ...
    public void handle(final JobClient client, final ActivatedJob job) {
      ...
      if (failJob) {
        ...
        client.newCompleteCommand(job.getKey()).payload((String) null).send().join();
      }
    }
  }
}
